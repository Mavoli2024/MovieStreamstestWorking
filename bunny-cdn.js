...
class BunnyCDN {
    ...
    /**
     * Update CDN configuration with fallback support
     */
    ...
    /**
     * Get URL with fallback support for better reliability
     */
    constructVideoUrlWithFallback(source) {
        const primaryUrl = this.constructVideoUrl(source);
        const fallbackUrls = this.fallbackUrls.map(fallbackBase => {
            const baseUrl = fallbackBase.endsWith('/') ? fallbackBase : fallbackBase + '/';
            const filename = source.startsWith('/') ? source.substring(1) : source;
            return baseUrl + filename;
        });
        
        return [primaryUrl, ...fallbackUrls].filter(url => url);
    }
